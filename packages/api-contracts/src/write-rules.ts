...
export const TABLE_CHECKS: Record<string, string> = {
  ...
  "brand_canvas_edges_no_self_loop": "source_item_id <> target_item_id",
  "brand_canvas_items_gen_medium": "ref_kind <> 'gen' or medium is not null",
  "brand_canvas_items_iframe_source": "case when ref_kind = 'iframe' then (url is null) <> (html is null) else url is null and html is null end",
  "brand_canvas_items_iframe_url_scheme": "url is null or url ~* '^https?://'",
  "brand_canvas_items_ref_kind_check": "ref_kind in ('post', 'media', 'document', 'memory', 'graphic', 'note', 'gen', 'iframe')",
  "brand_canvas_items_ref_shape": "(ref_kind in ('note', 'iframe') and ref_id is null) or (ref_kind = 'gen') or (ref_kind not in ('note', 'gen', 'iframe') and ref_id is not null)",
