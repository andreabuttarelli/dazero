...
### Un checkbox il cui `onchange` non parte mai, mentre `.checked` cambia lo stesso
Cliccando un `<input type="checkbox">` con `checked={expr}` (binding a una via) più `onchange={…}`, il DOM aggiorna `.checked` (verificato con `isChecked()`) ma l'handler non viene mai chiamato — nessun log, nessun `pageerror`, nessuna eccezione server. Riprodotto con `onchange`, `onclick`, un `id`/`for` espliciti al posto del nesting implicito nella `<label>`, e `bind:group` (che fallisce diversamente: in SSR crasha con `Cannot read properties of undefined (reading 'includes')` se l'array legato non esiste ancora lato server — un `$effect` che lo inizializza non gira mai durante l'SSR, solo dopo l'hydration). Non risolto in 8 giri di debug con tracing sia server che browser. Segnale: `checkbox.isChecked()` torna `true` dopo il click, ma nessun log dentro l'handler che quello stesso click dovrebbe chiamare compare mai. Mossa presa: non inseguire oltre un budget fisso — verificare la funzione SERVER con un test che chiama l'action vera mockando solo il confine esterno (qui: il client Zernio), che prova la parte che conta (il collegamento click → server → provider) senza passare dal browser; lasciare il difetto di interazione client-side come follow-up esplicito, mai nascosto in un test disabilitato in silenzio.

### Un bottone cliccato subito dopo `page.goto`/`waitForURL` non ha ancora il suo `onclick`
Su una pagina SvelteKit renderizzata server-side, il DOM del bottone esiste — Playwright lo vede `visible`, lo clicca, nessun errore — ma se il click arriva prima che l'hydration client-side abbia agganciato gli handler, il click cade su un nodo ancora "morto": nessuna eccezione, nessun log in console, nessun `pageerror`, e la funzione che quel bottone dovrebbe chiamare (qui: `openSheet` dietro un bottone della rail) semplicemente non parte — l'URL non cambia, niente si apre. A differenza del checkbox qui sopra, questo caso SI risolve: uno script standalone con `page.on('console')`/`page.on('pageerror')` attivi, ripetuto con e senza un `await page.waitForLoadState('networkidle')` dopo la navigazione, ha isolato la causa in due minuti — senza quell'attesa il click non fa niente in ogni run, con quell'attesa funziona in ogni run. Segnale: un click che Playwright riporta come riuscito (nessuna eccezione dal `.click()` stesso) ma il cui effetto atteso (URL, DOM, stato) non arriva mai, e zero rumore in console o server. Mossa: `waitForLoadState('networkidle')` (o l'equivalente `gotoHydrated` già in `fixtures/session.ts`) dopo OGNI navigazione che precede un click, non solo dopo il login — la stessa corsa esiste su qualunque pagina, non solo sul form che l'ha fatta scoprire per primo.
