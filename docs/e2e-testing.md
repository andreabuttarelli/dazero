# Testing E2E su feega, senza andare a tentativi
...
```bash
git worktree add -b fix/<slug> ../feega-wt/<slug> dev
cd ../feega-wt/<slug>
cp /Users/andreabuttarelli/Documents/GitHub/feega/.env .env
...
La stack porta `feega-app` (immagine `feega-selfhost-app`, porta 3000),
`feega-kong` (8000), `feega-db` (5432), `feega-cron`. Attenzione:
`feega-app` prosciuga `chat_jobs` dallo stesso DB del tuo dev server — se
...
```bash
docker images | grep feega-selfhost-app   # data dell'immagine vs git log -1
docker logs --since 1h feega-app 2>&1 | grep -i chat
```
...
dentro `feega-kong`. Il dev server del worktree deve parlare con la stack
...
```bash
getenv() { docker inspect feega-app --format '{{range .Config.Env}}{{println .}}{{end}}' \
...
- **utente**: `test@feega.app` / `123456`
- **brand**: slug `demo`, nome `Demo Brand`, piano `pro`, attivo

Verifica che esistano (idempotente, mai ricreare a mano):

```bash
docker exec feega-db psql -U postgres -d postgres -c \
  "select id, email from auth.users where email = 'test@feega.app';
...
PUBLIC_SUPABASE_URL=http://localhost:8000 \
SUPABASE_SERVICE_ROLE_KEY=$(getenv SUPABASE_SERVICE_ROLE_KEY) \
SEED_DEMO_EMAIL=test@feega.app SEED_DEMO_PASSWORD=123456 \
...
await page.locator('input[name="email"]').fill('test@feega.app');
...
1. **Il redirect finale va all'ultimo brand usato**, non a `/app`: il cookie
   `feega_last_brand` può mandarti su `/app/<vecchio-brand>`. Naviga sempre
...
- [ ] stack docker su, `feega-app` fresco o fermato consapevolmente
- [ ] dev server su porta esplicita, env overlay con `localhost:8000`, log su file
- [ ] utente `test@feega.app` e brand `demo` presenti (una query psql)
