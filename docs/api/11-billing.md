...
```bash
curl -sS -X POST https://feega.app/api/v1/brands/demo/billing/portal \
  -H "Authorization: Bearer $FEEGA_TOKEN" \
...
```bash
curl -sS -X POST https://feega.app/api/v1/brands/demo/billing/checkout \
  -H "Authorization: Bearer $FEEGA_TOKEN" \
