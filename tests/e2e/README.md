...
già seminati. `canvas.spec.ts`, `settings-brand.spec.ts` e `assets.spec.ts` sono `@real` di un
altro tipo: non seminano niente a mano, si costruiscono ognuno la propria org/progetto/tela
usa-e-getta con `tests/e2e/fixtures/session.ts` (stesso pattern di `scripts/eval/canvas.ts` e
...
E2E_REAL_STACK=1 E2E_BASE_URL=http://localhost:5173 npx playwright test canvas.spec.ts settings-brand.spec.ts assets.spec.ts onboarding.real.spec.ts
```
