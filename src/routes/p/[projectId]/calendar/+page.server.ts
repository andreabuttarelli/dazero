...
export const load: PageServerLoad = async (event) => {
  ...
  const url = event.url;
  const { brand: brandOrNull } = await event.parent();
  ...
  const page = await cachedBrandPage(event, brand.slug, async () => {
...
export const actions: Actions = {
  ...
  editPost: async ({ request, params, fetch, locals }) => {
    ...
    const brandSlug = await brandSlugOf(locals.supabase, params.projectId);
    ...
    const res = await fetch(brandApi(brandSlug, `/posts/${encodeURIComponent(id)}`), {
      method: 'PUT',
      headers: {
  ...
  approvePost: async ({ request, params, fetch, locals }) => {
    ...
    const res = await fetch(brandApi(brandSlug, `/posts/${encodeURIComponent(id)}/approve`), {
      method: 'POST',
