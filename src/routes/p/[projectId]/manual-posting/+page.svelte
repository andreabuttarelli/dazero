<script lang="ts">
  ...
  async function writeWithAi() {
    ...
    error = '';
    try {
      const res = await fetch(`/p/${page.params.projectId}/manual-posting/generate`, {
        method: 'POST',
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch(`/p/${page.params.projectId}/manual-posting/create`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          platforms: selected,
      ...
      if (body.noAccount) {
        ...
        await goto(`/p/${page.params.projectId}/calendar`);
        return;
      }
      await goto(`/p/${page.params.projectId}/calendar`);
    } catch {
    ...
    <section class="mp-col">
      <div class="mp-card">
        ...
        {#if selected.some((p) => !connected.has(p))}
          <a class="mp-link" href={`/p/${page.params.projectId}/settings/connected-accounts`}>{$_('app.manualPosting.connectCta')} →</a>
        {/if}
      </div>
...
<style>
  ...
  .drop {
    ...
    padding: 28px 14px; border: 1.5px dashed var(--line-2, #d2d2d7);
  ...
  .lib-toggle {
    ...
    border: 1.5px solid var(--line, #ededef); padding: 7px 12px;
  ...
  .lib-cell {
    aspect-ratio: 1; border: 1.5px solid var(--line, #ededef);
  ...
  textarea, input[type='text'], input[type='date'], input[type='time'] {
    ...
    border: 1.5px solid var(--line, #ededef); padding: 10px 12px;
  ...
  .when-opt {
    text-align: left; border: 1.5px solid var(--line, #ededef);
  ...
  .spin {
    width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.35); border-top-color: #fff; animation: mp-spin 0.8s linear infinite; display: inline-block;
  }
