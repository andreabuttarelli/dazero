...
export const GET: RequestHandler = async ({ params, url, locals: { supabase, safeGetSession } }) => {
  ...
  if (!adsAvailable(brand.plan, user?.email)) throw redirect(303, `/p/${params.projectId}/settings/ads/accounts`);
  ...
  const accountsUrl = `/p/${params.projectId}/settings/ads/accounts`;
  ...
  try {
    ...
    if (zernioPlatform === 'facebook' && (/404|400|no .*facebook|parent|posting|linked_account/i.test(msg))) {
      throw redirect(
        303,
        `/p/${params.projectId}/settings/connect/facebook?return=${encodeURIComponent('settings/ads/accounts')}`
      );
    }
