 ...
 * POST   → METTE IN CODA (`enqueueLoop`), con lo stesso gate crediti di `generate/+server.ts`:
 *          sopra 50 combinazioni senza `confirm: true` torna `needs_confirmation` (200, non un
 *          errore — è una domanda, non un rifiuto), sopra 1000 `refused` (400) qualunque cosa
 *          `confirm` dica. NON gira niente: il cron (`canvas/runs/tick`, ogni minuto) drena la
 *          coda nei minuti successivi — un loop grande finisce in più tick, mai per metà per
 *          sempre dentro il tetto di UNA richiesta.
 * DELETE → cancella (`cancelLoop`): ferma solo i biglietti non ancora reclamati da un tick, i
 *          risultati già completati restano.
 */
...
export const POST: RequestHandler = async ({ request, params, url }) => {
  ...
  const outcome = await enqueueLoop(db, {
    orgId,
...
export const DELETE: RequestHandler = async ({ request, params, url }) => {
  ...
  const nodeId = params.id ?? '';
  const outcome = await cancelLoop(db, { orgId, nodeId });
  return json(outcome);
};
