<script lang="ts">
  ...
  const currentNavHref = $derived.by(() => {
    ...
    const match = NAV.flatMap((g) => g.items)
      .map((item) => ({ item, normalised: item.href.replace(/\/$/, '') }))
  ...
  const pager = $derived.by(() => {
    ...
    const toLink = (item: NavItem | undefined) =>
      item ? { title: item.title, href: item.href } : null;
    return {
  ...
  const searchItems = $derived.by((): DocsSearchItem[] => {
    ...
    for (const group of [...DOCS_NAV, ...API_NAV]) {
      for (const item of group.items) {
        out.push({ title: item.title, href: item.href, group: group.label });
      }
    }
    return out;
  });
  ...
  const breadcrumbJsonLd = $derived.by(() => {
    ...
    for (const part of parts) {
      acc += `/${part}`;
      const title =
        searchItems.find((s) => s.href === acc || s.href.endsWith(acc))?.title ??
        part.replace(/-/g, ' ');
      items.push({
        name: title,
        item: `${origin}${acc}`
  ...
  function onMobilePageChange(href: string) {
    pageMenuOpen = false;
    if (href !== currentNavHref) goto(href);
  }
...
<div class="flex min-h-screen pt-14 max-lg:pt-[6.25rem]">
  ...
  <aside class={cn(
    ...
    <div class="mt-auto px-5 py-4 border-t border-border">
      <a href={'/docs/api'} class="flex items-center gap-2 text-[13px] text-muted-foreground no-underline hover:text-foreground transition-colors">
