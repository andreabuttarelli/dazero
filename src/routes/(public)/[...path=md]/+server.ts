 ...
 *
 * The pages are Svelte components, so the markdown is derived from the SSR'd HTML rather than
 * kept as a second source of truth — a new docs page is mirrored the moment it exists, with no
 * duplicate content to keep in sync.
 */
export const GET: RequestHandler = async ({ params, url, setHeaders }) => {
  const pagePath = `/${params.path.slice(0, -3)}`;

  // Deliberately the global fetch, not event.fetch: no cookies.
