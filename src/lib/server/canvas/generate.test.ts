 ...
 * ACCESO — mai a insaputa dell'utente, mai sul testo (`prompt-enhance.ts` non ha craft per il
 * testo). Il fallimento della riscrittura non blocca il giro: genera con il prompt originale.
 */
describe('params.enhancePrompt riscrive il prompt prima di generare', () => {
  ...
  it('acceso: passa il prompt riscritto al fornitore, non l\'originale', async () => {
    llmText.mockResolvedValue({ text: 'a photorealistic cat, studio lighting', citations: [] });
  ...
  it('spento: non chiama llmText, usa il prompt originale', async () => {
    const imageNodeRow = { ...freshNodeRow, type: 'image' };
    const { db } = fakeDb(
      { nodes: [imageNodeRow], assets: [] },
      { updateRows: { nodes: [{ ...imageNodeRow, version: 2 }] } }
  ...
  it('la riscrittura fallisce: genera comunque, con il prompt originale', async () => {
    llmText.mockRejectedValue(new Error('gateway down'));
