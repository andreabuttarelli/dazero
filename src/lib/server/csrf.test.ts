...
const url = (path: string) => new URL(`https://www.dazero.co${path}`);

function req(init: { method?: string; type?: string; origin?: string } = {}) {
  const headers = new Headers();
  if (init.type) headers.set('content-type', init.type);
  if (init.origin) headers.set('origin', init.origin);
  return new Request('https://www.dazero.co/x', { method: init.method ?? 'POST', headers });
}
...
describe('isCsrfForbidden', () => {
  ...
  it('allows a same-origin form POST, charset and casing included', () => {
    const same = { type: `${FORM}; charset=UTF-8`.toUpperCase(), origin: 'https://www.dazero.co' };
