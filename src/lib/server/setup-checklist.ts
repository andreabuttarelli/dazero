...
// The brand-setup todo list — the SAME items the sidebar OnboardingChecklist shows, so the
...
export async function buildSetupChecklist(supabase: SupabaseClient, brandId: string): Promise<SetupChecklist> {
  const [{ data: brand }, { data: kit }, { count: products }, { count: history }, { count: documents }, { data: gtm }, { data: plan }] =
    await Promise.all([
      supabase.from('brands').select('slug, blog_config').eq('id', brandId).maybeSingle(),
