/**
 * IL SUPERVISORE IN-LOOP. Guarda la stessa sequenza di tool che vede il modello e interviene
 * quando un agente batch (strategy, week_planner, produce, ugc_plan, media_generator, ...)
 * ripete una chiamata che fallisce, gira in tondo, o salta una lettura del brand prima di una
 * ricerca a pagamento. Le note sono visibili al modello (una patch nel system del prossimo step
 * e/o un tool negato), mai un secondo modello: un critico ad ogni step raddoppierebbe latenza e
 * costo e discuterebbe col modello principale.
 *
 * I tool bloccati tornano un'istruzione strutturata (`blocked_by: 'steward'`), mai `{ error }`:
 * i modelli leggono `error` come un'interruzione di servizio e riprovano la stessa ricerca.
 */
...
/**
 * Tracciato in memoria per il ciclo di un agente batch — solo ciò che serve allo steward per
 * ricostruire lo snapshot (nome, chiamate, esiti, passo corrente) e per riscrivere il system
 * prompt di ogni step. Niente persistenza: vive e muore col turno.
 */
export class AgentToolLog {
	readonly agent: string;
	private calls: string[] = [];
	private results: StewardSnapshot['results'] = [];
	private step = 0;
	private system = '';
	...
	recordResult(name: string, output: unknown, ok: boolean, error?: string): void {
		this.results.push({ name, failed: resultLooksFailed(output, ok, error), error });
	}
	...
	snapshot(tools: string[]): StewardSnapshot {
		return { agent: this.agent, tools, calls: [...this.calls], results: [...this.results], step: this.step };
	}
}

export type AgentSteward = {
...
export type AgentToolBeforeHook = (ctx: {
	name: string;
	input: unknown;
}) => Promise<{ deny?: string; result?: unknown } | void> | { deny?: string; result?: unknown } | void;

export type AgentToolAfterHook = (ctx: { name: string; input: unknown; output: unknown }) => Promise<unknown | void> | unknown | void;

export type AgentToolPipeline = {
	before?: AgentToolBeforeHook[];
	after?: AgentToolAfterHook[];
};

/** Uno steward legato a un log di agente. Chiama `stewardWouldBlock` prima di ogni tool e nega quando serve. */
export function createAgentSteward(log: AgentToolLog, toolNames: string[]): AgentSteward {
	const tools = [...toolNames];
	const snap = () => log.snapshot(tools);
		...
		pipeline: () => ({
			before: [
				({ name }) => {
					...
					return {
						deny: block.code,
						result: stewardDenyResult(block, name, block.code === 'search_before_brand' ? reads[0] : undefined)
					};
				}
			]
		})
...
export function mergePipelines(a?: AgentToolPipeline, b?: AgentToolPipeline): AgentToolPipeline | undefined {
...
/** Applica le note dello steward al prossimo step: patch nel system, tool ristretti se serve. */
export function applyStewardPrepareStep(
	...
	if (patch) {
		const systemBase =
			typeof base.system === 'string' && base.system ? String(base.system) : origSystem || log.currentSystem();
