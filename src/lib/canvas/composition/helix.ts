...
export const params: LayoutParam[] = [
	...
	{ name: 'height', label: 'Altezza', kind: 'range', min: 1, max: 40, step: 0.5, default: 8 },
	...
	{ name: 'scale', label: 'Scala', kind: 'range', min: 0.1, max: 4, step: 0.05, default: 1.15 },
	{ name: 'pulse', label: 'Pulsazione', kind: 'range', min: 0, max: 1.5, step: 0.05, default: 0.25 }
];
...
export function transforms(count: number, rawParams: LayoutParams, t: number): Transform[] {
	...
	return Array.from({ length: count }, (_, index) => {
		...
		const angle = progress * turns * FULL_TURN + spin;
