...
export function apply(pixels: Pixels, params: Record<string, number | string>): Pixels {
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			const [srcX, srcY] = pinchSource(x, y, centerX, centerY, radius, strength);
...
function pinchSource(x: number, y: number, centerX: number, centerY: number, radius: number, strength: number): [number, number] {
	const dx = x - centerX;
	const dy = y - centerY;
	...
	const pull = Math.pow(factor, strength >= 0 ? 2 : 0.5) * strength;
	const scale = 1 - pull * factor;
