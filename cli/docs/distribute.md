# Distribute the dazero CLI (npm + Homebrew)

## Install channels

| Channel | Command |
|---------|---------|
| **npm** | `npm install -g dazero-cli` |
| **Homebrew** | `brew tap andreabuttarelli/tap https://github.com/andreabuttarelli/homebrew-tap && brew install dazero` (tap repo created once, see below) |
...
(npm Access Token with publish rights for `dazero-cli`).

One-time npm setup:

1. Create the package scope/name on [npmjs.com](https://www.npmjs.com) if needed (`dazero-cli`).
2. Create a granular access token (read/write for `dazero-cli`) or classic automation token.
...
Formula: [`Formula/dazero.rb`](../Formula/dazero.rb). A Homebrew tap must be its own
...
1. Builds `dazero-<platform>` binaries and `.tar.gz` archives
...
```bash
brew update && brew upgrade dazero
```
