...
console.log(`Building dazero CLI for: ${selectedTargets.map(t => t.name).join(', ')}\n`);

// Build each target
for (const target of selectedTargets) {
  const outName = `dazero-${target.name}`;
...
console.log('\nTo install locally:');
console.log('  sudo cp dist/dazero-macos-arm64 /usr/local/bin/dazero');
console.log('\nTo distribute:');
