...
describe('i modelli media come contratto', () => {
      ...
      slots: [
        {
          ...
          model: null,
          choices: [
            { id: 'nano-banana-pro', label: 'Nano Banana Pro', aspectRatios: ['1:1', '4:5'], maxRefs: 8 }
          ]
        }
      ]
    });
  ...
  // I limiti sono il motivo per cui questa lettura esiste prima di una scrittura o di un render:
  // un modello scelto senza sapere quanto dura o quali rapporti serve si scopre dal rifiuto.
  it('una scelta senza i suoi limiti non passa', () => {
    const parsed = GET_MEDIA_MODELS.output.safeParse({
      brand: 'demo',
      slots: [
        {
          slot: 'videoModel',
          job: MEDIA_MODEL_JOBS.videoModel,
          model: null,
          choices: [{ id: 'bytedance/seedance-2-5', label: 'Seedance 2.5' }]
        }
      ]
    });
