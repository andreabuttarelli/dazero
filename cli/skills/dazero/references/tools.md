...
session is the CLI's, so `dazero login` in a terminal covers both. `list_brands` is how you
...
| you want | how |
|---|---|
| posts, by status | `query` on `posts` — `id`, `status`, `platform`, `caption`, `scheduled_for`, `published_at`, `created_at`; CLI `dazero content <slug> [--status …]` |
...
| the calendar | `query` on `posts` — `id`, `platform`, `caption`, `scheduled_for`, `slot`, `status`, filtered `scheduled_for` `is` null with `negate: true`, ordered ascending; CLI `dazero calendar <slug> [--month YYYY-MM]` |
| the brand at a glance | `dazero dashboard <slug>` and `dazero status <slug>` — no single table stands in for them |
| what publishing achieved | `dazero analytics <slug>` |
| the media library | `query` on `brand_media` — `id`, `kind`, `mime`, `title`, `description`, `tags`, `short_code`, `created_at`; the link to hand out is `https://dazero.co/a/<short_code>` |
...
| the editorial plan and its weeks | `query` on `editorial_plans` — `id`, `status`, `strategy`, `voice`, `cadence`, `platform_mix`, `weeks`, `created_at`, `activated_at`, filtered `status` `eq` `active`; CLI `dazero plan <slug>` and `dazero weekly-plan <slug>` |
| the go-to-market | `dazero gtm <slug>` |
| how goal mode went | `GET /api/v1/brands/:slug/goals` — met on the first pass, laps spent, `stopped_by` |
| how the brand is supposed to sound, and its settings | `query` on `brands` — `slug`, `name`, `plan`, `status`, `timezone`, `target_platforms`, `content_prefs`, `blog_config` with `limit: 1`; voice, hashtags and blog settings all live in those two jsonb columns. CLI `dazero voice <slug>` |
...
| the studio | `query` on `products`, `people`, `competitors` and `brand_kit`; CLI `dazero studio <slug>`, `dazero products <slug>`, `dazero people <slug>` |
...
| SEO / GEO audits | `query` on `brand_geo_audits` — `id`, `created_at`, `tech_score`, `tech`, `share_of_voice`, `citations`, newest first; CLI `dazero seo <slug>` and `dazero geo <slug>` |
| the fixes those audits produced | `query` on `brand_geo_artifacts` — `id`, `kind`, `title`, `format`, `body`, `status`, `target_path`, `source_finding` |
| keyword strategy | `query` on `brand_seo_keyword_strategy` — `strategy`, `citations`, `updated_at` with `limit: 1`; CLI `dazero keywords <slug>` |
...
| articles | `query` on `brand_articles` — `id`, `slug`, `title`, `status`, `scheduled_for`, `published_at`, `created_at`; CLI `dazero web <slug>` |
...
Seven reads are NOT a query, because not one of them is a select: `list_brands`,
`diagnose_brand`, `search_knowledge`, `get_writing_skills`, `get_creation_kit` and
...
the id `create_post` accepts. Required: `slug`, `url`; optional `title`. dazero copies the file
...
`create_post` stores copy **you** wrote: dazero calls no model and spends no credits. It does
...
`check_content` runs the checks dazero runs on its own copy against a spec you wrote, before
...
It reads the copy and nothing else. **No tool here judges an image or a video** — dazero stopped
...
| MCP | CLI |
|-----|-----|
| `approve_plan` | `dazero plan <slug> approve` |
| `save_brief` | `dazero plan <slug> save-brief --week N …` |
| `produce_week` | `dazero weekly-plan <slug> produce --week N` |
| `save_plan` | (MCP only) |
| `save_week_seeds` | (MCP only) |

Reading the plan back is `query` on `editorial_plans` — `strategy`, `voice`, `cadence`,
`platform_mix` and `weeks`, filtered `status` `eq` `active` — or `dazero plan <slug>` and
`dazero weekly-plan <slug>`, which both still answer.
...
| MCP | CLI |
|-----|-----|
| `update_brand_identity` | `dazero studio <slug> kit-update\|colors …`, `dazero voice <slug>` |
...
| `research_competitors` | `dazero studio <slug> research` |
| `set_bio` | (MCP only) |
| `sync_history` | `dazero studio <slug> sync-history` |

What the studio holds is read with `query`, one table at a time: `products`, `people`,
`competitors` and `brand_kit`, each row with its id. `dazero studio <slug>` still prints the
...
default `/blog/<slug>` address that page is served from dazero's own origin, alongside the
...
| `ads_action` | `dazero ads <slug> [--propose\|--create\|--approve\|--pause\|--resume\|--duplicate\|--delete\|--reject] [--ad <adId>]` |
| `ads_remix` | (MCP only) |
...
`scheduled_for`. dazero calls no model and spends no credits; nothing is rewritten or
